...
- [x] PRD_W2 해당 요구사항 확인 (chunking 모듈)
- [x] Duration 산정 확인

### 4.2 요구사항 분석
- [x] 노트 → N개 청크 분할 전략 정의 (문단/토큰 단위)
- [x] 청크 크기/오버랩 정책 (max_tokens, overlap_tokens)
- [x] 비동기 분할 트리거 시점 정의 (노트 저장/갱신 시)
- [x] 청크 재생성 정책 (노트 갱신 시 기존 청크 삭제 후 재생성)
- [x] Instructions 초안 → TASK 문서 반영

### 4.3 Security 1차 검토
- [x] 인증 필요 여부: 내부 서비스 호출 (Spring event 기반, 직접 API 노출 없음)
- [x] 권한 종류: 시스템 내부 처리
- [x] 공개 API 여부: No
- [x] 결과 → TASK Constraints 반영

### 4.4 ERD 설계
- [x] note_chunks 테이블 설계 (id, note_id, chunk_index, chunk_text, token_count, embedding vector(1536), created_at)
- [x] 참고: 임베딩은 별도 테이블(chunk_embeddings)이 아닌 note_chunks.embedding 컬럼에 저장
- [x] 인덱스 설계 (note_chunks.note_id, note_chunks.note_id+chunk_index UNIQUE)
- [x] 관계 정의 (note_chunks.note_id → notes.id FK)
- [x] Duration(final) 갱신

### 4.5 Security 2차 검토
- [x] 청크 데이터 접근 제어 (내부 서비스만)
- [x] 노트 삭제 시 관련 청크 cascade 삭제 확인
- [x] 행 단위 접근 제어: 노트 소유자 기반 간접 제어
- [x] 결과 → TASK Constraints 반영

### 4.6 DTO / Entity 설계 (API First)
- [x] ChunkResponse 정의 (id, note_id, chunk_index, chunk_text, token_count)
- [x] ChunkCreateEvent 정의 (note_id, chunk_text)
- [x] NoteChunk Entity 작성
- [x] MapStruct 매퍼 작성
- [x] Output Format → TASK 반영

### 4.7 Repository 구현
- [x] NoteChunkRepository 인터페이스 작성
- [x] findByNoteIdOrderByChunkIndex 커스텀 쿼리
- [x] deleteByNoteId 벌크 삭제 쿼리
- [x] Flyway 마이그레이션 스크립트 작성

### 4.8 Service + Test
- [x] ChunkingService 구현 (chunkNote, getNoteChunksByNoteId, deleteNoteChunksByNoteId)
- [x] 노트 → N개 청크 분할 로직 구현 (문단/토큰 기반)
- [x] 비동기 처리 구현 (@Async 또는 Kafka Consumer)
- [x] 청크 재생성 로직 (기존 삭제 → 새 청크 생성)
- [x] 토큰 카운팅 유틸리티 구현
- [x] 단위 테스트 작성 (Mockito — 다양한 노트 길이)
- [x] 경계값 테스트 (빈 노트, 매우 긴 노트, 한국어 포함)
- [x] 테스트 통과 확인

### 4.9 Controller + Test
- [x] 내부 서비스 — 외부 Controller 해당 없음
- [x] 관리용 청크 조회 엔드포인트 (선택): 이번 Step Scope에서 제외
- [x] 통합 테스트 (노트 입력 → 청크 분할 결과 검증)
- [x] 테스트 통과 확인
