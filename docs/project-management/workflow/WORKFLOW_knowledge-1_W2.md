...
- [x] PRD_W2 해당 요구사항 확인 (graph 모듈)
- [x] Duration 산정 확인

### 4.2 요구사항 분석

- [x] note_links 기반 양방향 조회 요건 분석 (outgoing/incoming 링크)
- [x] 백링크 조회 API 요건 정의 (특정 노트를 참조하는 노트 목록)
- [x] D3.js 시각화용 노드/엣지 JSON 데이터 구조 정의
- [x] 그래프 탐색 깊이 제한 (depth) 정책 정의
- [x] Instructions 초안 → TASK 문서 반영

### 4.3 Security 1차 검토

- [x] 인증 필요 여부: Yes (JWT 인증 필요)
- [x] 권한 종류: 로그인 사용자 (본인 노트 그래프만)
- [x] 공개 API 여부: No
- [x] 결과 → TASK Constraints 반영

### 4.4 ERD 설계

- [x] note_links 테이블 확인/갱신 (sourceNoteId, targetNoteId, createdAt)
- [x] 양방향 조회를 위한 인덱스 설계 (sourceNoteId, targetNoteId)
- [x] 복합 유니크 제약 (sourceNoteId + targetNoteId)
- [x] Duration(final) 갱신

### 4.5 Security 2차 검토

- [x] 그래프 데이터 노출 범위 제한 (본인 노트만)
- [x] 깊이 제한으로 과도한 탐색 방지
- [x] 행 단위 접근 제어: 필요 (노트 소유자 확인)
- [x] 결과 → TASK Constraints 반영

### 4.6 DTO / Entity 설계 (API First)

- [x] BacklinkResponse 정의 (noteId, title, createdAt)
- [x] GraphNodeDto 정의 (id, title, group)
- [x] GraphEdgeDto 정의 (source, target)
- [x] GraphDataResponse 정의 (nodes[], edges[])
- [x] NoteLink Entity 확인/갱신
- [x] MapStruct 매퍼 작성
- [x] Output Format → TASK 반영

### 4.7 Repository 구현

- [x] NoteLinkRepository 인터페이스 작성/갱신
- [x] findByTargetNoteId (백링크 조회) 커스텀 쿼리
- [x] findBySourceNoteId (아웃링크 조회) 커스텀 쿼리
- [x] 재귀 그래프 탐색 쿼리 (depth 제한 CTE)
- [x] Flyway 마이그레이션 스크립트 작성 (필요 시)

### 4.8 Service + Test

- [x] GraphService 구현 (getBacklinks, getOutlinks, getGraphData)
- [x] 백링크 조회 서비스 (targetNoteId → source 노트 목록)
- [x] 아웃링크 조회 서비스 (sourceNoteId → target 노트 목록)
- [x] D3.js용 노드/엣지 JSON 생성 서비스 (depth 제한 BFS/DFS)
- [x] 소유자 검증 로직 구현
- [x] 단위 테스트 작성 (Mockito)
- [x] 테스트 통과 확인
...
- [x] GET /api/v1/notes/{noteId}/backlinks 엔드포인트 구현
- [x] GET /api/v1/notes/{noteId}/outlinks 엔드포인트 구현
- [x] GET /api/v1/graph?noteId={id}&depth={n} 엔드포인트 구현 (D3.js 데이터)
- [x] 슬라이스 테스트 (@WebMvcTest)
- [x] 401/403 응답 테스트
- [x] 통합 테스트 (양방향 링크 + 그래프 JSON 검증)
- [x] 테스트 통과 확인
